 ...
 *
 * Runtime contract: a browser + a POST endpoint (the form `action`). CLIENT
 * validation is native HTML5 only -- it does NOT ship zod to the browser. FULL
 * `~standard` validation runs on the SERVER at POST; the script routes the
 * returned issues to each control's error slot by path.
 */
...
// default export as `(options) => Handler`.
